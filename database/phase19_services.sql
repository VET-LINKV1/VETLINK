...
--
-- Adds:
--   * services table — replace hardcoded REASONS array in bookingService.js
--   * RLS policies — admin CRUD, clients read active
--   * Seed with current hardcoded reasons
-- ============================================================

-- ── services table ─────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.services (
  id                  UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  code                TEXT NOT NULL UNIQUE,           -- e.g. 'annual_checkup'
  label               TEXT NOT NULL,                  -- e.g. 'Annual Check-up'
  description         TEXT,                           -- longer description for clients
  urgency             TEXT NOT NULL DEFAULT 'standard'
                        CHECK (urgency IN ('routine','standard','urgent','emergency')),
  duration_mins       INT NOT NULL DEFAULT 30,        -- default slot length
  suggested_specialty TEXT,                           -- e.g. 'Surgery', 'Emergency & Critical Care'
  color               TEXT NOT NULL DEFAULT 'slate'
                        CHECK (color IN ('blue','emerald','violet','amber','red','slate')),
  price               NUMERIC(10,2),                  -- optional, for display/quotes
...
-- Helper: is_clinical_staff (admin, staff, or veterinarian)
...
AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.users u WHERE u.id = uid AND u.role IN ('admin','staff','veterinarian')
  );
$$;
...
-- Clients (and vets): read only active services
...
-- ── View for booking API (matches existing REASONS shape) ───────
